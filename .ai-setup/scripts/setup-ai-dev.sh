...
REPO_URL="https://github.com/DEFRA/defra-ai-setup"
BRANCH="main"
...
echo "📦 Cloning defra-ai-setup repository (v${SCRIPT_VERSION})..."
# Force fresh clone to avoid caching issues
git clone --depth 1 --branch "$BRANCH" "$REPO_URL" "$TEMP_DIR/defra-ai-setup" 2>/dev/null || {
    echo "❌ Error: Failed to clone repository. Please check your internet connection."
    exit 1
}

# Check if there's a newer version available
if [ -f "$TEMP_DIR/defra-ai-setup/.ai-setup/scripts/setup-ai-dev.sh" ]; then
    REMOTE_VERSION=$(grep "^SCRIPT_VERSION=" "$TEMP_DIR/defra-ai-setup/.ai-setup/scripts/setup-ai-dev.sh" | cut -d'"' -f2)
    if [ "$REMOTE_VERSION" != "$SCRIPT_VERSION" ]; then
        echo ""
        echo "⚠️  A newer version of this script is available (v${REMOTE_VERSION})"
        echo "   Please download the latest version from:"
        echo "   curl -fsSL https://raw.githubusercontent.com/DEFRA/defra-ai-setup/refs/heads/main/.ai-setup/scripts/setup-ai-dev.sh | bash"
        echo ""
    fi
fi

echo "📋 Copying .ai-setup folder..."
if [ -d "$TEMP_DIR/defra-ai-setup/.ai-setup" ]; then
    cp -r "$TEMP_DIR/defra-ai-setup/.ai-setup" .
